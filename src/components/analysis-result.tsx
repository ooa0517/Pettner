...
import { 
  ...
  Dna, UtensilsCrossed, Weight, InfoIcon as InfoLucide
} from 'lucide-react';
...
export default function AnalysisResult({ result, input, onReset, resetButtonText }: AnalysisResultProps) {
  const { t } = useLanguage();
  const [productName, setProductName] = useState(result?.productIdentity?.name || '분석된 제품');
  ...
  const InfoTooltip = ({ content }: { content: string }) => (
    <Tooltip>
      ...
      <TooltipContent className="max-w-[250px] p-3 text-xs leading-relaxed bg-white border shadow-xl">
        {content}
      </TooltipContent>
    </Tooltip>
  );
  ...
  return (
    <TooltipProvider>
      <div className="space-y-10 animate-in fade-in duration-700 pb-40 max-w-4xl mx-auto px-4">
        ...
        {/* 1. Pet Summary Section (NEW: 아이 상태 우선) */}
        <Card className="border-none shadow-2xl rounded-[2.5rem] overflow-hidden bg-gradient-to-br from-primary/5 to-indigo-50/30 ring-1 ring-black/5">
           <CardHeader className="pb-2">
              ...
              <CardTitle className="text-2xl font-black flex items-center gap-2">
                 <PetIcon className="w-6 h-6 text-primary" /> {petSummary.statusMessage}
              </CardTitle>
           </CardHeader>
