...
export default function AnalysisResult({ result, input, onReset }: AnalysisResultProps) {
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-10 duration-1000 pb-40 max-w-4xl mx-auto px-4">
      ...
      {/* 3. Feeding Guide Dashboard */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { icon: Flame, label: '하루 필요 열량', value: result.feedingGuide.dailyKcal, color: 'text-orange-500' },
          { icon: UtensilsCrossed, label: '하루 급여량', value: result.feedingGuide.dailyAmount, color: 'text-primary' },
      ...
      <Accordion type="single" collapsible className="space-y-4">
        <AccordionItem value="nutrition" className="border-none shadow-lg bg-white rounded-[2rem] px-8">
          <AccordionTrigger className="hover:no-underline font-black text-lg py-6">
            <div className="flex items-center gap-3"><Zap className="text-primary"/> 영양 성분 상세 (DM 기준)</div>
          </AccordionTrigger>
          <AccordionContent className="pb-10 space-y-8">
             <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <BenchmarkBar label="단백질 농도" position={result.advancedNutrition.benchmarks.protein.position} value={result.advancedNutrition.dm_protein} />
                <BenchmarkBar label="지방 농도" position={result.advancedNutrition.benchmarks.fat.position} value={result.advancedNutrition.dm_fat} />
                <BenchmarkBar label="탄수화물 농도" position={result.advancedNutrition.benchmarks.carbs.position} value={result.advancedNutrition.dm_carbs} />
             </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-white/80 backdrop-blur-xl border-t z-50 flex justify-center">
        <div className="w-full max-w-4xl flex gap-4">
          <Button onClick={onReset} variant="outline" className="flex-1 h-16 rounded-2xl border-2 font-black">
            <Repeat className="mr-2 w-5 h-5" /> 다시 분석
          </Button>
          <Button 
